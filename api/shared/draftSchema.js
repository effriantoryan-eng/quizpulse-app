...
// Validates one question's shape (no count check — that's a whole-draft concern). Returns an
// array of { index, message } errors (empty when valid). Shared by validateDraftQuestions and by
// the regenerate-question endpoint, which validates exactly one question at a time.
function validateQuestionShape(q, index, { chunkCount } = {}) {
  const errors = [];
...
function validateDraftQuestions(questions, { chunkCount } = {}) {
  ...
  questions.forEach((q, index) => {
    errors.push(...validateQuestionShape(q, index, { chunkCount }));
  });
...
module.exports = {
  validateDraftQuestions, validateQuestionShape,
  MIN_QUESTIONS, MAX_QUESTIONS, MAX_TEXT_CHARS, MAX_OPTION_CHARS, OPTION_COUNT,
};
